...
const Contact = () => {
  ...
  const getPhoneHref = () => {
    ...
    if (typeof phone === 'object') {
      return `tel:${phone.en.replace(/ /g, '')}`;
    }

    // Handle old string format
    return `tel:${phone.replace(/^0/, '+66').replace(/ /g, '')}`;
  };
  ...
  const contactInfo = [
    {
      icon: Phone,
      label: t('about.phone'),
      value: getPhoneNumber(i18n.language),
      href: getPhoneHref()
    },
    {
      icon: Mail,
            ...
            <div className="space-y-4">
              {contactInfo.map((info, index) => (
                <motion.a
                  ...
                  viewport={{ once: true }}
                  className="flex items-center space-x-3 sm:space-x-4 p-3 sm:p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 group"
                >
                  <div className="flex-shrink-0">
                    <div className="w-10 h-10 sm:w-12 sm:h-12 bg-primary-100 rounded-lg flex items-center justify-center group-hover:bg-primary-200 transition-colors duration-200">
                      <info.icon className="w-5 h-5 sm:w-6 sm:h-6 text-primary-600" />
                    </div>
                  </div>
                  <div className="min-w-0 flex-1">
                    <p className="text-xs sm:text-sm text-gray-500 font-medium">{info.label}</p>
                    <p className="text-gray-800 font-semibold text-sm sm:text-base truncate">{info.value}</p>
                  </div>
                </motion.a>
              ))}
            </div>
